...
public class Menu {
    ...
    private static void initializeSeedMenu() {
        ...
        StdDraw.text(Game.WIDTH / 2, 15, "Please Insert Your Seed");
    }
    ...
    /**
     * Provides the user directions for entering a seed
     */
    ...
    private static void selectCharacterScreen() {
        ...
        for (int i = 0; i < 1;) {
            if (StdDraw.hasNextKeyTyped()) {
                menuChoice = Character.toLowerCase(StdDraw.nextKeyTyped());
                if (menuChoice == 's') {
                    Tileset.setPLAYER(COLORS[colorIndex]);
                    i++;
    ...
    private static void displayCharacter() {
        ...
        StdDraw.text(82, Game.HEIGHT / 2, "⇨");
        StdDraw.setFont(new Font("Monaco", Font.PLAIN, 65));
        StdDraw.text(22, (Game.HEIGHT / 2) - 1.5, "A");
        StdDraw.text(78, (Game.HEIGHT / 2) - 1.30, "D");
        StdDraw.setFont(new Font("Monaco", Font.PLAIN, 15));
        StdDraw.setPenColor(Color.WHITE);
