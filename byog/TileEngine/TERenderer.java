...
public class TERenderer {
    ...
    private void updateHUD() {
        ...
        StdDraw.text(Game.WIDTH - 2.3, Game.HEIGHT + 0.9, tile.toUpperCase());
        StdDraw.text(Game.WIDTH / 2 + 1.4, Game.HEIGHT + 0.9, "x" + numKeys);
        StdDraw.picture(Game.WIDTH / 2 - 0.6, Game.HEIGHT + 0.9, "/byog/Media/key.png", 4.5, 3.25);
        double startPos = 1;
        for (int i = 0; i < numHearts; i++) {
            StdDraw.picture(startPos, Game.HEIGHT + 0.9, "/byog/Media/heart.png", 1.5, 1.5);
            startPos += 1.6;
        }

    }

    public void pendingTileType(String tile) {
        this.tile = tile;
    }

    public void pendingKeys(int keys) {
        this.numKeys = keys;
    }

    public void pendingHearts(int hearts) {
        this.numHearts = hearts;
    }

    public void pendingLineOfSight(int x, int y) {
        playerXPos = x;
        playerYPos = y;
    }

    private void updateLineOfSight() {
        StdDraw.picture(playerXPos, playerYPos, "/byog/Media/hole1.png", 320, 220);
    }
